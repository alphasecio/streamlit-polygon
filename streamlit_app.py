...
def add_goals_tracking(df):
    ...
    with col1:
        ...
        leads_progress = (actual_leads / lead_goal) * 100
    ...
    with col2:
        ...
        appointments_progress = (actual_appointments / appointment_goal) * 100
    ...
    with col3:
        ...
        closings_progress = (actual_closings / closing_goal) * 100
...
def calculate_metrics(df):
    ...
    metrics = {
        ...
        'Lead to Appointment Rate': (df['appointments'].sum() / df['leads'].sum() * 100),
        'Appointment to Close Rate': (df['closings'].sum() / df['appointments'].sum() * 100),
        'Overall Close Rate': (df['closings'].sum() / df['leads'].sum() * 100),
...
with tab1:
    ...
    # Create sample data
    sample_data = {
        'month': pd.date_range(start='2023-01-01', periods=12, freq='M').strftime('%Y-%m-%d').tolist(),
        'leads': np.random.randint(80, 120, 12).tolist(),
        'appointments': np.random.randint(40, 60, 12).tolist(),
        'closings': np.random.randint(20, 30, 12).tolist()
    }
